...
class ConvolutionalNeuralNetworkModel:
    def __init__(self):
        ...
        conv_2 = tf.nn.bias_add(conv2d(pool_1, W2), b2)
        pool_2 = maxpool2d(conv_2)

        logits = tf.nn.bias_add(tf.matmul(tf.reshape(pool_2, [-1, 7 * 7 * 64]), W3), b3)
        logits_2 = tf.nn.bias_add(tf.matmul(tf.reshape(logits, [-1, 1024]), W4), b4)

        # Predictor
        f = tf.nn.softmax(logits_2)

        # Cross Entropy loss
        self.loss = tf.losses.softmax_cross_entropy(self.y, logits_2)
